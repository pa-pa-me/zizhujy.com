...
(function ($) {
    ...
    function setAxesVisibility(plot, show) {
        var options = plot.getOptions();
        // set axis.show = true will cause problem:
        // flot will met a error complaining box is undefined.
    ...
    function handleRatioXY(plot) {
        ...
        if (options.coordinate.ratioXY) {
            ...
            if (!xaxis.options.max) xaxis.options.max = (xaxis.max || 10);
            if (!xaxis.options.min) xaxis.options.min = (xaxis.min || -10);
            var deltaX = xaxis.options.max - xaxis.options.min;
            var deltaY = deltaX / (ratio * plot.width() / plot.height());

            yaxis.options = yaxis.options || {};
            if (!yaxis.options.max) yaxis.options.max = (yaxis.max || 10);
            if (!yaxis.options.min) yaxis.options.min = (yaxis.min || -10);
