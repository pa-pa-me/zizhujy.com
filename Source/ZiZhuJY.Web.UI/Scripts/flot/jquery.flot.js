...
(function ($) {
    ...
    function Plot(placeholder, data_, options_, plugins) {
        ...
        plot.getSurface = function () { return surface; };
        ...
        function drawGrid() {
            ...
            if (markings) {
                ...
                for (i = 0; i < markings.length; ++i) {
                    var m = markings[i];
                    var aboveGrid = typeof m.aboveGrid !== "undefined" ? m.aboveGrid : options.grid.markingsAboveGrid || false;
        ...
        function drawMarking(m, markingLayer) {
            ...
            if (xrange.from === xrange.to || yrange.from === yrange.to) {
                ...
                if (rounded === 0) {
                    ...
                    roundRect(ctx, xrange.from, yrange.to, xrange.to - xrange.from, yrange.from - yrange.to, rounded);
                    ctx.fill();
                }
            }

            if (m.texts && m.texts.length > 0) {
                // text added to markings is an array of text objects, each element of the 
                // text array is called a text block
                var blocks = m.texts;
                var blockCount = blocks.length;

                for (var k = 0; k < blockCount; k++) {
                    // each block holds a block of text, which may contain '\r\n'. If there are
                    // '\r\n's in a block, it will produce some extraHeight.
                    var extraHeight = 0;
                    var currentBlock = blocks[k];
                    // margins may be specified in the text object in the format of 
                    ...
                    var xPos = (xrange.from + plotOffset.left + xrange.to + plotOffset.left) / 2;
                    var yPos = (yrange.from + plotOffset.top) + (yrange.to - yrange.from) * (blockCount - k) / (blockCount + 1);
                    yPos += extraHeight;

                    // a block of text may have multiple lines caused by the the '\r\n's. If there is no
                    // '\r\n' in the block, then the block is a single line.
                    var lines = currentBlock.text.toString().split(/[\r\n]+/);
                    ...
                    if (currentBlock.font) {
                        ...
                        if (!!realFontSize) {
                            realFontSize = parseFloat(realFontSize);
                        }

                        font.family = realFont[realFont.length - 1] || "Arial";
                        font.size = parseFloat(realFont[realFont.length - 2]) || "10";
                        font.lineHeight = font.size;
                        font.weight = realFont[realFont.length - 3] || "normal";
                        font.variant = realFont[realFont.length - 4] || "normal";
                        font.style = realFont[realFont.length - 5] || "normal";
                    } else {
                        var autoFont = ctx.font.split(/\s+/);
                        font.family = autoFont[autoFont.length - 1] || "Arial";
                        font.size = parseFloat(autoFont[autoFont.length - 2]) || "10";
                        font.lineHeight = font.size;
                        font.weight = autoFont[autoFont.length - 3] || "normal";
                        font.variant = autoFont[autoFont.length - 4] || "normal";
                        font.style = autoFont[autoFont.length - 5] || "normal";
                    }

                    font.color = currentBlock.textColor || "#fff";

                    if (!!currentBlock.textAlign) {
                        switch (currentBlock.textAlign.toLowerCase()) {
                            case "left":
                                xPos = xrange.from + plotOffset.left + marginLeft;
                                break;
                            case "right":
                                xPos = xrange.to + plotOffset.left - marginRight;
                                break;
                            default:
                                break;
                        }
                    ...
                    if (!!currentBlock.textBaseline) {
                        switch (currentBlock.textBaseline.toLowerCase()) {
                            case "top":
                                yPos = (yrange.to + plotOffset.top + marginTop) + font.lineHeight * k;
                                break;
                            case "bottom":
                                yPos = (yrange.from + plotOffset.top - marginBottom) - font.lineHeight * (blockCount - 1 - k);
                                break;
                            default:
                                break;
                        }
                    ...
                    yPos -= font.lineHeight * (lines.length - 1) / 2;

                    for (var l = 0 ; l < lines.length; l++) {
                        surface.addText(markingLayer, xPos, yPos, lines[l], font, 0, null, currentBlock.textAlign || "center", currentBlock.textBaseline || "middle");
                        extraHeight += font.lineHeight;
    ...
    //      the rectangle to be drawn.
    // @param {number} w The width of the rectangle to be drawn.
    // @param {number} h The height of the rectangle to be drawn.
