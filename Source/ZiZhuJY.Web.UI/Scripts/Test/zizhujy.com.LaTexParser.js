;(function (LaTexLex) {
    ...
    if(typeof zizhujy.com.lex== "undefined") zizhujy.com.LaTexParser = {
        version: 1.0,
        source: "",
        errorList: [],
        ...
        lex: function() {
            ...
            if(buildNewLeaf) this.newLeafForTree();
        ...
        nodeTypeEnum: {
            ...
            OTHER_LEXEME: {value: 25, name: "Other Lexeme"},
                            ...
                            switch(this.nodeType) {
                                ...
                                case zizhujy.com.LaTexParser.nodeTypeEnum.RHS_YT:
                                    ...
                                    for(var i = 0; i < this.children.length; i++) {
                                        ...
                                        this.isValid = this.isValid && this.children[i].isValid;
                                    }
                                ...
                                case zizhujy.com.LaTexParser.nodeTypeEnum.LATEX_EXP_YT:
                                    this.isValid = this.isValid && this.children[0].isValid;
                                    switch(this.children[0].expression){
                                        case "\\frac":
                                            this.isValid = this.isValid && this.children[2].isValid;
                                            this.isValid = this.isValid && this.children[5].isValid;
        ...
        newLeafForTree: function() {
            new this.TreeNode(this.nodeTypeEnum.OTHER_LEXEME, this.nextToken.lexeme, this.currentNode);
        },
        ...
        /// </summary>
        /*[obselete]*/
        explicitFunctionLaTex: function(){
            this.log("Enter <显函数LaTex>");
            // parse $
            if(this.nextToken.token == LaTexLex.TokenEnum.DOLLAR_SIGN){
                this.lex();
                if(this.nextToken.token != LaTexLex.TokenEnum.DOLLAR_SIGN) {
                    // parse function body
                    while(this.nextToken.token == LaTexLex.TokenEnum.SEMICOLON) this.lex();
                    if(this.nextToken.token != LaTexLex.TokenEnum.DOLLAR_SIGN) {
                        this.functionBody();
                    
                        while(this.nextToken.token == LaTexLex.TokenEnum.SEMICOLON) {
                            this.lex();
                            if(this.nextToken.token != LaTexLex.TokenEnum.SEMICOLON && this.nextToken.token != LaTexLex.TokenEnum.DOLLAR_SIGN)
                                this.functionBody();
                        }

                        if(this.nextToken.token != LaTexLex.TokenEnum.DOLLAR_SIGN) 
                            this.error(this.errorEnum.DOLLAR_SIGN_EXPECTED);
                    }else{
                        // Done
                        this.lex();
                    }
                ...
                this.error(new this.Exception(this.errorEnum.DOLLAR_SIGN_EXPECTED));
            this.log("Exit <显函数LaTex>");
        },
        ...
        priorityFactor_yt: function() {
            ...
            switch(this.nextToken.token){
                case LaTexLex.TokenEnum.OPEN_PARENTHESIS:
                    this.lex(true);
                    this.RHS_yt();
                    if(this.nextToken.token == LaTexLex.TokenEnum.CLOSING_PARENTHESIS) this.lex();
                    else
                        this.error(this.errorEnum.CLOSING_PARENTHESIS_EXPECTED);
                    break;
                case LaTexLex.TokenEnum.LATEX_NAME:
