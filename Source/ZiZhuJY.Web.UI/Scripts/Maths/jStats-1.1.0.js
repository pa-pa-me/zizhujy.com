...
(function (Matrix) {
    ...
    function jStats() {
        ...
        if (arguments.length <= 0) {
            ...
            if (arguments[0] instanceof JPoints2D) {
                for (var i = 0; i < arguments[0].points.length; i++) {
                    this.dataCenter.x.push(arguments[0].points[i].x);
                    this.dataCenter.y.push(arguments[0].points[i].y);
                }
            } else {
        ...
        this.statVars = {
            ...
            a: 0,
            b: 0,
            r: 0,   // correlation co-efficient A measure of the strength of linear association between two variables. Correlation will always between -1.0 and +1.0. If the correlation is positive, we have a positive relationship. If it is negative, the relationship is negative.
            // Formula: Correlation Co-efficient :            Correlation(r) =[ NΣXY - (ΣX)(ΣY) / Sqrt([NΣX2 - (ΣX)2][NΣY2 - (ΣY)2])] 
            // Where 
            /*
        ...
        if (!this._initialized) {
            ...
            jStats.prototype.analyse = function () {
                ...
                this.statVars.r = (this.statVars.n * this.statVars.Sum_XY - this.statVars.SumX * this.statVars.SumY) / Math.sqrt((this.statVars.n * this.statVars.Sum_X_2 - Math.pow(this.statVars.SumX, 2)) * (this.statVars.n * this.statVars.Sum_Y_2 - Math.pow(this.statVars.SumY, 2)));
