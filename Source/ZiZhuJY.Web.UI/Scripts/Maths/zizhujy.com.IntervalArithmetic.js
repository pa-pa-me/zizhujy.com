...
//            }
        ...
        pow: function(interval1, interval2){
            if(interval2.val[0] == interval2.val[1]){
                ...
                if(a > 1){
                    ...
                    throw "Don't know how to calculate it yet. arguments: " + JSON.stringify(interval1) + "; " + JSON.stringify(interval2);
                }
            } else {
                throw "Don't know how to calculate it yet. arguments: " + JSON.stringify(interval1) + "; " + JSON.stringify(interval2);
            }
        ...
        greaterThan: function (interval1, interval2) {
            ...
            if (interval1.val[0] === interval2.val[0] && interval1.val[1] === interval2.val[1] &&
                interval1.def[0] === true && interval1.def[1] === true &&
                interval2.def[0] === true && interval2.def[1] === true) {
                return {
                    val: [false, false],
                    def: [true, true]
                };
            ...
            return {
                val: [
                    interval1.val[0] > interval2.val[1],
                    interval1.val[1] > interval2.val[0]
                ],
        ...
        // This equal would bring problems when drawing y=x.
        equals2: function(interval1, interval2){
            if (this.include(interval1, interval2).val.equals([true, true])
                && this.include(interval2, interval1).val.equals([true, true])) {
                return { val: [true, true], def: [true, true] };
            } else if(this.greaterThan(interval1, interval2).val.equals([true, true])){
                return {val: [false, false], def: [true, true]};
            } else if (this.greaterThan(interval2, interval1).val.equals([true, true])) {
                return { val: [false, false], def: [true, true] };
            } else {
        ...
        equals:function(left, right) {
            var r1 = this.greaterThan(left, right);
            var r2 = this.greaterThan(right, left);

            r1 = this.not(r1);
            r2 = this.not(r2);

            var result = this.and(r1, r2);
            return result;
        },
    ...
    window.IntervalArithmetic = zizhujy.com.TupperIntervalArithmetic;
})(window);