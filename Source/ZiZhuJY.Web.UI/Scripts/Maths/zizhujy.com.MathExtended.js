    ...
    function Fraction() {
        ...
        function _buildFractionFromRepeatingDecimal(firstPart, period) {
            ...
            if (typeof Big === 'function') {
                denominator = new Big(basic.denominator).times(Math.power(10, fractalLengthOfFirstPart));
                numerator = new Big(firstPart).times(denominator).plus(basic.numerator);
            }
            
            if (typeof math === 'object') {
                denominator = math.eval(basic.denominator + ' * 10 ^ ' + fractalLengthOfFirstPart);
                numerator = math.eval(firstPart + ' * ' + denominator + ' + ' + basic.numerator);
            }
        ...
        function _buildFraction() {
            ...
            if (typeof arguments[0] !== "number") {
                if (typeof arguments[0] === 'string' && !repeatingDecimalRegex.test(arguments[0])) {
                    return this.fromFiniteDecimal(arguments[0], math || null);
                } else {
        ...
        // New interfaces:
        function fromFiniteDecimal(decimal, math) {
            var s = decimal.toString();
            var decimalCount = s.substr(s.indexOf('.') + 1).length;
            var denominator = Math.pow(10, decimalCount);
            var numerator = decimal * denominator;

            if (math) {
                denominator = math.eval('10 ^ ' + decimalCount);
                numerator = math.eval(s + ' * ' + denominator);
            }
        ...
        return {
            ...
            // New interfaces:
            fromFiniteDecimal: fromFiniteDecimal
        };
    }
