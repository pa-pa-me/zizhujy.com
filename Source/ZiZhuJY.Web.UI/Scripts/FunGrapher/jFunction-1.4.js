...
//
// Version: 1.3
// Copyright (c) 2013 - jeff@zizhujy.com
...
(function ($, parser) {
    ...
    function jExplicitFunction() {
        ...
        if (!this._initialized) {
            ...
            /// <summary>
            ///     Generate data points using Adaptive Plotting method
            /// </summary>
            /// <param name="start">Domain start, required</param>
            /// <param name="end">Domain end, required</param>
            /// <param name="rangeStart">required</param>
            /// <param name="rangeEnd">required</param>
            /// <param name="canvasWidth">required</param>
            /// <param name="canvasHeight">required</param>
            /// <param name="step">optional</param>
            jExplicitFunction.prototype.generateDataPoints = function(start, end, rangeStart, rangeEnd, canvasWidth, canvasHeight) {
                function interpolate(data, rhs, threshold){
                    if(data.length >= 2){
                        var i = data.length - 2;
                        var j = data.length - 1;
                        if(arguments.length > 3) i = arguments[3];
                        if(arguments.length > 4) j = arguments[4];

                        if(data[i] != null && data[j] != null){
                            var midYOnLine = (data[i][1] + data[j][1])/2;
                            var midXOnLine = (data[i][0] + data[j][0])/2;
                            var midX = (data[i][0] + data[j][0])/2;
                            var x = midX;
                            var midY = eval(rhs);
                            var diffY = Math.abs(midY - midYOnLine);
                            var deltaY = Math.abs(midYOnLine - data[i][1]);
                            var deltaX = Math.abs(midXOnLine - data[i][0]);

                            if(diffY > threshold){      
                                if(deltaY / deltaX < Math.tan(Math.PI * (19999999999/20000000000)) ){
                                    // Insert [midX, midY] into data just before the last point
                                    data.splice(j, 0, [midX, midY]);

                                    // No improve for graph quality
                                    // Left interpolate
                                    interpolate(data, rhs, threshold, data.length-3, data.length-2);                                

                                    // Right interpolate
                                    interpolate(data, rhs, threshold);
                                }else{
                                    data.splice(j, 0, null);
                                }
                            }
                        }
                    }
                ...
                var pixelWidth = (end-start)/canvasWidth;
                var pixelHeight = (rangeEnd - rangeStart)/canvasHeight;
                var step = pixelWidth;
                if(arguments.length > 6 && arguments[6] != undefined) step = arguments[6];
                ...
                switch(this.getFunctionItemType()) {
                    case jFunctionItemType.fx:
                        var count = 0;
                        for(var i = start; i<= end; i+= step, count++) {
                            x = i;
                            y = eval(this.rhs);
...
//                            if(data.length > 1){
//                                var deltaY = Math.abs(data[data.length-1][1] - data[data.length-2][1]);
//                                var deltaX = Math.abs(data[data.length-1][0] - data[data.length-2][0]);

//                                if(deltaY/deltaX >= Math.tan(Math.PI/2)){
//                                    data.push(null);
//                                }
//                            }

                            interpolate(data, this.rhs, pixelHeight);
                        }
                        break;
...
//                s = s.replace(re, "$1*$2");
    ...
    function jFunction(expression) {
        ...
        this.isValid = true;
        // This is used to calculate the roots of the f(x) = 0 || f(y) = 0 || f(t) = 0 who should be excluded when plot the graph of the function
        this.denominators = [];
        ...
        if (!this._initialized) {
            ...
            jFunction.prototype.generateDataPoints = function(start, end, dependentMin, dependentMax, canvasWidth, canvasHeight, step) {
                if(this.functionObject != null) {
                    //return this.functionObject.generateDataPoints(n, start, end, dependentMin, dependentMax);
                    return this.functionObject.generateDataPoints(start, end, dependentMin, dependentMax, canvasWidth, canvasHeight, step);
                }else {
                    return null;
                }
            };
    ...
    function jFunctionList(input) {
        ...
        if (!this._initialized) {
            ...
            jFunctionList.prototype.analyse = function () {
                function getDenominators(subTree){
                    var denominators = [];
                    var start = false;
                    for(var i = 0; i < subTree.children.length; i++){
                        if(subTree.children[i].nodeType == parser.nodeTypeEnum.OTHER_LEXEME && 
                            subTree.children[i].evalCache == "/" && !start){
                            start = true;
                            continue;
                        }

                        if(start){
                            denominators.push(subTree.children[i]);
                            start = false;

                            continue;
                        }

                        var d = getDenominators(subTree.children[i]);
                ...
                for(var i = 0; i < parser.tree.children.length; i++) {
                    if(parser.tree.children[i].nodeType == parser.nodeTypeEnum.FUNCTION_BODY && parser.tree.children[i].getExpression() != ";") {
                        ...
                        func.denominators = getDenominators(parser.tree.children[i]);
