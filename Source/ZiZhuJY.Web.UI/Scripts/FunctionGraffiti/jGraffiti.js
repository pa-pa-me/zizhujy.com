        ...
        if (!this._initialized) {
            ...
            jGraffiti.prototype.buildUI = function () {
                ...
                if (this.ui.$InputBoard.attr("value").length <= 0) {
                    var defaultFunctions = "y = sin(10*x);\ny = cos(2*x);\ny = x ^ 3;\ny = x/3;\ny = -x/3;\ny = 0;\nx = 0;\n{x = 5*cos(t); y = 5*sin(t)}";
            ...
            //
            jGraffiti.prototype.processUserInput = function (s) {
                // 替换中文符号
                ...
                // 识别幂函数输入：
                //s = s.replace(/(\w+)\s*\^\s*([-\+]?\s*\w+|\(.*\))/g, "pow($1, $2)");
                var rePower = /(\w+?(?:\([^\^]*?\))??|\([^\^]*?\))\s*?\^\s*?([-\+]??\s*?\w+?|\(.*?\))/g;
                while (rePower.test(s)) {
                    s = s.replace(rePower, "pow($1, $2)");
                    rePower.lastIndex = 0;
                }
                //s = s.replace(rePower, function ($1) { return $1.replace(rePower, "pow($2, $3)"); });
                //alert(s);
