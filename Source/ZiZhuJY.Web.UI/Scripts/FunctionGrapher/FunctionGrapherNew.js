    ...
    $(document).ready(function () {
        ...
        FunGrapher.UI = function(){
            ...
            function addExpressionLine($target, expression) {
                ...
                // When html = "<div class='expressionLine'><div id='taExp-d783b49c-27d3-2db5-5a26-90fc7e77b0d5' class='expressionBox'><span class='mathquill-editable mathquill-rendered-math hasCursor' mathquill-block-id='576'><span class='textarea'><textarea></textarea></span><span class='cursor'>‍</span></span></div></div>"
                // There would be problem when using $(html) with jquery-1.4.4.js to jquery-1.8.3.js, and to jquery-1.9.0.js!!!
                // So I have to find another way....
                /*
                var html = "<div class='expressionLine'><div id='taExp-" + guid() + "' class='expressionBox'><span class='mathquill-editable'>{0}</span><span class='icon clear'>×</span></div></div>".format(!!expression ? expression : "");
                */
                ...
                //$expressionLine.find("span.mathquill-editable").mathquill("editable").focus();
                $expressionLine.find("span.mathquill-editable").mathquill("editable").mathquill("latex", !!expression ? expression : "").focus();

                // Workaround for focusing on IE:
                setTimeout(function () {
                    $expressionLine.find("span.mathquill-editable").find("textarea").focus();
                }, 100);

                $expressionLine.find("span.mathquill-editable textarea, span.mathquill-editable input")
                    .bind("keydown", myKeyDown)
                    .bind("keyup", myKeyUp)
                    .bind("blur", myBlur)
                    .keyup();

                // Google Nexus 7 Chrome has some typing issues due to its IME
            ...
            function deleteExpressionLine ($mathquill) {
                ...
                //deleteErrorMessageBox($mathquill.closest("div.expressionBox").attr("id"));
            ...
            function deleteAllExpressionLines(){
                var $expressionLines = $("#leftPanel div.expressionLine").not(":first")
                                                    .each(function(index, expressionLine){
                                                        //deleteErrorMessageBox($(expressionLine).find("div.expressionBox").attr("id"));
                                                        $(expressionLine).removeAttr("data");
                                                        plotExpressionLine($(expressionLine));
                                                        removeErrorStyle($(expressionLine));
                                                     }).remove();
                //deleteErrorMessageBox($("#leftPanel div.expressionLine:first div.expressionBox").attr("id"));
                
                // Don't understand why the below statement will make the mathquill edit box be very narrow,
                // which the $clearButton argument passed to the clearExpression is differ from the click event on only
                // the jQuery context is different
                //clearExpression($("#leftPanel div.expressionLine:first").find(".clear"));
            ...
            function plotExpressionLine($expressionLine){
                ...
                if (plot == null) {
                    ...
                    if (!!oldData && oldData instanceof Array) {
                        ...
                        oldData = oldData.filter(function (s) { if (!!s && !!s.color) delete s.color; return !!s && !!s.data && (typeof s.data.id != "undefined") && s.data.id != oldSeriesId; });
                    }
                ...
                isDrawing = false;
                coor = new zizhujy.com.GeneralRectangularCoordinate(
            ...
            function showErrorStyle($expressionLine, ar) {
                ...
                $(String.format(newErrMsg, expressionId))
                        .html(getErrorMessage(ar.errorList))
                        .appendTo("body")
                        .css({
                            "right": right,
                            "top": $expressionLine.offset().top
            ...
            function autoSizeCanvasWrapper() {
                ...
                // width adjustment must be successive after the height adjustment, because the scroll bar width will
                // destroy the left-right layout.
                autoWidthControls();
                autoWidthCanvasWrapper();
            }
        ...
        FunGrapher.ExamplePanel.UI = function(){
            ...
            function hidePanel (){
                // >>
                $icon.text("\uf31a");
                $examples.animate({"right": (FunGrapher.UI.ControlPanelScrollBarWidth() - $content.outerWidth()) + "px"}, 500, function() {$content.hide();});
            }
        ...
        var options = {
            lines: { show: true },
            //points: { show: false, radius: 1, symbol: "pixel" },
        ...
        var coor = new zizhujy.com.GeneralRectangularCoordinate(
            plot.getOptions().xaxis.min,
            plot.getOptions().xaxis.max,
            plot.getOptions().yaxis.min,
            plot.getOptions().yaxis.max,
        ...
        function replotJob() {
            // Only 3 entry points set isDrawing = true
            //  1. At the very begining of all codes
            //  2. advancedHandleAnalyzedResult()
            //  3. handleAnalyzedResult();

            // Only 2 entry points set isDrawing = false
            //  1. plotExpressionLine();
            //  2. plotAllExpressionBoxes();
            if (!!isDrawing) {
                // Wait 2 seconds and back here to check again
            } else {
                // Check if replotting needed
                var currentViewPort = {
                    left: plot.getOptions().xaxis.min,
                    right: plot.getOptions().xaxis.max,
                    bottom: plot.getOptions().yaxis.min,
                ...
                if (!validViewPort || currentViewPort.left < validViewPort.left
                    ...
                    var curCoor = new zizhujy.com.GeneralRectangularCoordinate(
            plot.getOptions().xaxis.min,
            plot.getOptions().xaxis.max,
            plot.getOptions().yaxis.min,
            plot.getOptions().yaxis.max,
            ...
            plot.height());
                    ...
                    if ((curPixelWidth < pixelWidth / 3 || curPixelHeight < pixelHeight / 3) /*&& containsImplicitFunction()*/) {
                        replotAll();
                    } else {
                    }
                }
        ...
        //var mathquillKeyDown = $mathquill.keydown;
        $mathquill.bind("keydown", FunGrapher.UI.KeyDownHandler)
        .bind("keyup", FunGrapher.UI.KeyUpHandler)
        .bind("blur", FunGrapher.UI.BlurHandler);
        ...
        function analyzeInput(s) {
            ...
            //console.info(parser.logBuffer.toString("\n"));
            //console.info(!!tree ? tree.getExpression() : "");
            ...
            if (ar.isValid && !!tree) {
                ...
                if (tree.children.length == 3 || tree.children.length == 4) {
                    ...
                    if (tree.children[0].getLeaves().length == 1 && tree.children[0].getLeaves()[0].nodeType == parser.nodeTypeEnum.VARIABLE_NAME) {
                        ...
                        && !tree.children[2].getLeaves().filter(function (item) { return item.nodeType == parser.nodeTypeEnum.VARIABLE_NAME })
                                            .map(function (item) { return item.expression; }).contains("y")) {
                            ar.equationObject.type = "explicit function y = f(x)";
                        } else if (tree.children[0].getLeaves()[0].expression == "x"
                        && !tree.children[2].getLeaves().filter(function (item) { return item.nodeType == parser.nodeTypeEnum.VARIABLE_NAME })
                                            .map(function (item) { return item.expression; }).contains("x")) {
                    ...
                    && !tree.children[0].getLeaves().filter(function (item) { return item.nodeType == parser.nodeTypeEnum.VARIABLE_NAME })
                                        .map(function (item) { return item.expression; }).contains("y")) {
            ...
            //console.info("Equation Type: " + ar.equationObject.type);

            $.extend(ar.equationObject, {
                generateData: function () {
                    return sampleEquation(
                        ar.equationObject.type,
                        tree,
                        plot.width(),
                        plot.height(),
                        plot.getAxes().xaxis.options.min,
                        plot.getAxes().xaxis.options.max,
                        plot.getAxes().yaxis.options.min,
                        plot.getAxes().yaxis.options.max,
                        500,
                        !!tree ? parser.getConstraints(tree) : null
                        );
                }
            });
            ...
            function sampleEquation(type, tree, viewPortWidth, viewPortHeight, xStart, xEnd, yStart, yEnd, dataPoints, constraints) {
                ...
                switch (type) {
                    case "explicit function y = f(x)": 
                        ...
                        var independentVar = tree.children[2].getLeaves().filter(function (node) { return node.nodeType == parser.nodeTypeEnum.VARIABLE_NAME; });
                        if (!!independentVar && independentVar.length > 0) independentVar = independentVar[0].expression;
                        else {
                            independentVar = "x";
                        }
                        var dependentVar = tree.children[0].getLeaves().filter(function (node) { return node.nodeType == parser.nodeTypeEnum.VARIABLE_NAME; });
                        if (!!dependentVar && dependentVar.length > 0) dependentVar = dependentVar[0].expression;
                        else {
                            dependentVar = "y";
                        }
                        data = Plotter.sampleXY(
                            function (x) { return eval(tree.children[2].eval()); },
                    ...
                    case "explicit function x = f(y)":
                        ...
                        data = Plotter.sampleXY(
                            function (y) { return eval(tree.children[2].eval()); },
                            { min: yMin, max: yMax, step: step, tolerance: xtolerance }
                        ).swapCols(0, 1);
                        break;
                    case "explicit function f(x) = y":
                        var step = (xMax - xMin) / dataPoints;
                        var independentVar = tree.children[0].getLeaves().filter(function (node) { return node.nodeType == parser.nodeTypeEnum.VARIABLE_NAME; });
                        if (!!independentVar && independentVar.length > 0) independentVar = independentVar[0].expression;
                        else {
                            independentVar = "x";
                        }
                        var dependentVar = tree.children[2].getLeaves().filter(function (node) { return node.nodeType == parser.nodeTypeEnum.VARIABLE_NAME; });
                        if (!!dependentVar && dependentVar.length > 0) dependentVar = dependentVar[0].expression;
                        else {
                            dependentVar = "y";
                        }
                        data = Plotter.sampleXY(
                            function (x) { return eval(tree.children[0].eval()); },
                            { min: xMin, max: xMax, step: step, ytolerance: ytolerance }
                        );
                        break;
                    case "explicit function f(y) = x":
                        ...
                        data = Plotter.sampleXY(
                            function (y) { return eval(tree.children[0].eval()); },
                            { min: yMin, max: yMax, step: step, tolerance: xtolerance }
                        ).swapCols(0, 1);
                        break;
                    case "parameter function x=f(t);y=g(t)":
                        ...
                        var varX = tree.children[0].getLeaves().filter(function (node) { return node.nodeType == parser.nodeTypeEnum.VARIABLE_NAME; });
                        if (!!varX && varX.length > 0) varX = varX[0].expression; else varX = "x";

                        var varY = tree.children[4].getLeaves().filter(function (node) { return node.nodeType == parser.nodeTypeEnum.VARIABLE_NAME; });
                        ...
                        data = Plotter.sampleTXY(
                            function (t) { return eval(tree.children[2].eval()); },
                            function (t) { return eval(tree.children[6].eval()); },
                    ...
                    case "implicit function":
                        ...
                        data = Plotter.sampleImplicit(
                            ...
                            { xMin: xMin, xMax: xMax, yMin: yMin, yMax: yMax }, debug
                        );
        ...
        function basicHandleAnalyzedResult(ar, $expressionLine) {
            if (ar.isValid) {
                ...
                //console.info("error: " + ar.errorList.length);
            }
        }
        ...
        function advancedHandleAnalyzedResult(ar, $expressionLine) {
            ...
            if (ar.isValid) {
                ...
                if (type == "implicit function") {
                    $("#mask-layer").show();
                    implicitCalculations ++;
                    setTimeout(function () { handle(); FunGrapher.UI.PlotExpressionLine($expressionLine); }, 200);
                } else {
                    $("#mask-layer").show();
                    implicitCalculations ++;
                    handle();
        ...
        // Google Nexus 7 Chrome has some issues with position: fixed;
        var ua = navigator.userAgent.toLowerCase();
        var isAndroid = ua.indexOf("android") >= 0;
        if(isAndroid){
