namespace WebMarkupMin.Core.Parsers
{
	...
	internal sealed class XmlParser
	{
		...
		private static readonly Regex _processingInstructionRegex =
			new Regex(@"^<\?(?<instructionName>[\w\-:]+)" + 
				@"(\s+(?<attributes>(\s*[\w\-:]+\s*=\s*((?:""[^""]*?"")|(?:'[^']*?')))+))?" + 
				@"\s*\?>");
		private static readonly Regex _startTagRegex =
			new Regex(@"^<(?<tagName>[\w\-:]+)" +
				@"(\s+(?<attributes>(\s*[\w\-:]+\s*=\s*((?:""[^""]*?"")|(?:'[^']*?')))+))?" +
				@"\s*(?<emptyTagSlash>/?)>");
		private static readonly Regex _endTagRegex = new Regex(@"^<\/(?<tagName>[\w\-:]+)\s*>");
		private static readonly Regex _attributeRegex = new Regex(@"(?<attributeName>[\w\-:]+)" + 
			@"\s*=\s*" + 
		...
		private bool ProcessEndTag()
		{
			...
			if (match.Success)
			{
				...
				{
					if (_tagStack.Count(t => t.Name == endTagName) > 0)
					{
						throw new XmlParsingException(
							string.Format(Strings.ErrorMessage_NotClosedTag, stackedTag.Name),
							stackedTag.Coordinates,
