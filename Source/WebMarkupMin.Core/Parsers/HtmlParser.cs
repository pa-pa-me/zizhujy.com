...
namespace WebMarkupMin.Core.Parsers
{
	...
	internal sealed class HtmlParser
	{
		...
		private static readonly Regex _startTagRegex =
			new Regex(@"^<(?<tagName>[a-zA-Z0-9\-_:]+)(?<attributes>(?:\s*[a-zA-Z0-9\-_:]+(?:\s*=\s*" +
				@"(?:(?:""[^""]*?"")|(?:'[^']*?')|[^>""'\s]+)?)?" +
				@"\s*(?<invalidCharacters>[^a-zA-Z0-9\-_:>/]+|[""'=]+|(?:[/\s]+\s+))?" +
				@")*)\s*(?<emptyTagSlash>/?)>");
		private static readonly Regex _endTagRegex = new Regex(@"^<\/(?<tagName>[a-zA-Z0-9\-_:]+)\s*>");
		private static readonly Regex _attributeRegex =
			new Regex(@"(?<attributeName>[a-zA-Z0-9\-_:]+)(?:\s*(?<attributeEqualSign>=)\s*(" + 
			...
			SourceCodeNodeCoordinates attributesCoordinates, bool isEmptyTag)
		{
			...
			if (tagFlags.OptionalEndTag)
			{
				...
				{
					if (tagName == "body" && _tagStack.Count(t => t.Name == "head") > 0)
					{
						ParseEndTag("head");
					}
				}
		...
		private static bool IsUriBasedAttribute(string tagName, string attributeName, IEnumerable<HtmlAttribute> attributes)
		{
			...
			switch (attributeName)
			{
				...
				case "profile":
					...
					break;
				case "value":
					isUriBased = (tagName == "param" && attributes.Count(a => a.Name == "name"
						&& _uriBasedParams.Contains(a.Value.Trim().ToLowerInvariant())) > 0);
					break;
				default:
					isUriBased = false;
					break;
